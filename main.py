...
def read_config():
    ini_dict = {}
    file = 'conf/allconf.ini'
    con = configparser.ConfigParser()
    con.read(file, encoding='utf-8')
    ini_dict['bark_url_farceli'] = dict(con.items('bark_url'))['farceli']
    # ini_dict['har_WXHar'] = dict(con.items('har_path'))['WXHar']
    # ini_dict['har_QQHar'] = dict(con.items('har_path'))['QQHar']
    return ini_dict


print(read_config())
# # 初始化
# farceli_url = read_config()  # 初始化bark_url
#
...
#         report.add_row([title, for_list(month_game_list), len(month_game_list), Tool.red_text(str(number_of_timeouts))])
#     else:
...
#         bark_title = '王者荣耀当前周期战绩推送（beta）/'
#         bark_body = '当前周期：' + title + '\n游戏数量：' + str(len(month_game_list)) + '（上限：120）'
#         bark_url = farceli_url + bark_title + bark_body
